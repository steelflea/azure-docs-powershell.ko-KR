...
Az은 짧아진 명령과 안정성을 향상 시켰으며, Windows, macOS 및 Linux를 지원합니다. Az은 AzureRM의 동일한 기능을 제공하며 AzureRM에서 쉬운 마이그레이션 경로도 제공합니다.
...
Az는 새로운 모듈이므로 버전이 다시 설정했습니다. 첫 번째 안정적인 릴리스는 1.0이지만, 모듈은 2018년 11월 현재 AzureRm과 동등한 기능을 유지합니다.

## <a name="upgrade-to-az"></a>Az로 업그레이드

사용자는 새로운 `Az` 모듈로 업그레이드하는 것이 좋습니다. 이렇게 하려면 다음을 수행합니다.
