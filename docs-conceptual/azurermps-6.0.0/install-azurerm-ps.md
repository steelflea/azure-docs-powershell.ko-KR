...
Azure PowerShell 버전 6.0.0에는 PowerShell 버전 5.0 이상이 필요합니다. 이전 버전의 Azure PowerShell에서는 cmdlet을 실행하는 데 PowerShell 버전 3.0 _이상_이 필요했습니다. PowerShell 5.0으로 업그레이드하는 방법에 대한 자세한 내용은 [이 표](/powershell/scripting/setup/installing-windows-powershell?view=powershell-6#upgrading-existing-windows-powershell)를 참조하세요.
