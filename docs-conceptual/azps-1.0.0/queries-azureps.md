...
`Get-AzVM` cmdlet을 사용하여 계정에서 VM 목록을 쿼리합니다.
...
원하는 속성이 JSON 출력에 중첩된 경우 해당 속성의 전체 경로를 제공해야 합니다. 다음 예제에서는 `Get-AzVM` cmdlet에서 VM 이름 및 OS 형식을 선택하는 방법을 보여 줍니다.
