...
PSJob은 개별 프로세스로 실행되므로 Azure 연결을 공유해야 합니다. `Connect-AzAccount`로 Azure 계정에 로그인한 후 작업에 컨텍스트를 전달합니다.
...
`Get-Job` 및 `Get-AzVM`을 사용하여 작업 및 진행률을 언제든지 검사할 수 있습니다.
